...
library(plyr)
library (xts)
library(data.table, zoo)
library(scales)
library(ggplot2, xtable)
library(sandwich, lmtest)
library(igraph)
library(ggthemes, stargazer)
library(dplyr)
library(dtplyr)

Paths = c("/Users/luciarisuenonavarro/Desktop/Migration project/R Project - lucia /Analysis.R")
names(Paths) = c("ts", "?")
...

{
  if (Sys.info()[7] == "ts") {
    ...
    dat1 <- fread("/Users/luciarisuenonavarro/Desktop/Migration project/R Project - lucia /mymigration/migration_country_birth")
    dat2 <- fread("/Users/luciarisuenonavarro/Desktop/Migration project/R Project - lucia /mymigration/migration_country_previous_residence")
...
trying1 <- read_excel('/Users/luciarisuenonavarro/Desktop/europe excel clean 1.xlsx')
class(trying1)
trying1<- setDT(trying1)
#trying1 <-melt(trying1, measure.vars =c('Number'),variable.factor = F, value.name= 'Number' )
#long <- melt(trying1, measure.vars = c("TOTAL", "FEMALE", "MALE"), variable.name = "SEX", variable.factor = T, value.name = "Number")
#trying1<-select(trying1,-variable, -variable.1)
View(trying1)
setcolorder(trying1, 2)
trying1[is.na(trying1)] <- 0
save(trying1, file = "Graph Data.RData")
class(trying1)

g <- graph_from_data_frame(trying1)
#E(g)$SEX <- as.factor(long$SEX)
#E(g)$AGE <- as.factor(long$AGE)
E(g)$YEAR <- as.factor(trying1$YEAR)
E(g)$weight <- (trying1$Number+0.01)
...
##merge in gdp data
gdpdat <- fread("/Users/luciarisuenonavarro/Desktop/Migration project/R Project - lucia /data files/GDP.csv")
gdpdat <- gdpdat[,.(DEST = geo, YEAR = TIME_PERIOD, GDPpcPPS = OBS_VALUE)][YEAR > 2010]
wip1 <- gdpdat[regdat, on=.(DEST=DEST, YEAR=YEAR)]

##merge in gini data
ginidat <- fread("/Users/luciarisuenonavarro/Desktop/Migration project/R Project - lucia /data files/GINI.csv")
...
unempdat <- fread("/Users/luciarisuenonavarro/Desktop/Migration project/R Project - lucia /data files/UNEMPLOYMENT.csv")
...
data[is.na(data)] <- 0
save(data, file = "Regression Data.RData1")
...
model3 <- lm(Number ~ GDP + Unempl. + Gini + Lag_Number + 
               Betweenness + Eigenv_Centr. + Strength, data = tsdat)
se3 <- sqrt(diag(vcov(model3)))
...
#coeftest(model3, vcov = NeweyWest(mode32))
...
if (Sys.info()[7] == "ts") {
  ...
  stargazer(model1, model2, model3, se = list(robust_se_1, robust_se_2, robust_se_3),
            ...
            model.names = T,
            star.cutoffs = c(0.05, 0.01, 0.001))
            # notes = c("Newey & West HAC SE reported in Parentheses"),
            # notes.align = "r",
